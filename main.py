...
sttime = datetime.now().strftime("%Y-%m-%d__%H:%M:%S - ")
user = "pi"
...
def Check_Settings():
    if os.path.isfile("settings.conf"):
        user_settings = open("settings.conf","r")
        contact_method = user_settings.readlines()[0]
        print(contact_method.lower())
        if contact_method.lower() == "email":
            email_username = user_settings.readlines()[1]
            if email_username == "":
                print("No username storeed check your settings.conf file")
            else:
                print (email_username)
                email_password = user_settings.readlines()[2]
                if email_password == "":
                    print("No password has been saved")
        else:
